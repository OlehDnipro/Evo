...
//			tools::exitFatal(error + "\n\nThe file may be part of the additional asset pack.\n\nRun \"download_assets.py\" in the repository root to download the latest version.", -1);
		...
		if (pScene)
		{
			...
			Buffer vb = CreateBuffer(device, vbParams);
			Buffer ib = CreateBuffer(device, ibParams);
			
			SMapBufferParams vbMapParams = { GetMainContext(device), vb, 0, vBufferSize };
			SMapBufferParams ibMapParams = { GetMainContext(device), ib, 0, iBufferSize };
